...
""" This script takes geometry file (in any ASE-supported format)
    ...
    after they are calculated (outside of this script), builds 3rd order force constant matrix
    and calculates phonon self-energy gamma.

    Naturally Phono3py stores data in hdf5 format, but here, 
    gammas and eigenfrequencies at each q-point are extracted as a postpocessing.
    ...
    Adjustable parameters in the beginning of 'main' section:
        ...
        mesh = [21, 21, 1] - mesh of q-points to uotput gammas and frequencies for
        temperatures=[80]  - list of temperatures to calculate gammas for
        symprec=1e-5       - symmetry precision. It is not recommended to change it

        cutoff_pair_distance = 6.0 - cutoff distance for phonon-phonon interaction. 
                                     Displacements involvind atoms beyond cutoff will be excluded.
                                     It is recommended to start with lower values and increase it until convergence.
                                     The disppacements are enumerated in a such way that adding further ones keeps previous valid, 
...
def to_phonopy_atoms(structure, wrap=False):
    ...
    Returns
    phonopy_atoms: PhonopyAtoms - The PhonopyAtoms for the same structure as atoms
    """
...
#        write("displacements_all.xyz", to_Atoms(scell), append=True)
        ...
        if os.path.exists("phono3py-displacement-%05d/geometry.in" % (i+1)):
            if to_Atoms(scell) == read("phono3py-displacement-%05d/geometry.in" % (i+1)):
                print("phono3py-displacement-%05d/geometry.in exists and is valid." % (i+1))
                continue
        write("phono3py-displacement-%05d/geometry.in" % (i+1), to_Atoms(scell))

    print("%i displacements are included." % (len(scells_with_disps) - excluded))
...
#                disp2['included']))
    ...
    unique_distances = np.unique(distances_int) * unique_thres # 1e-5 up to 5 decimals
...
if __name__ == '__main__':
    ...
    print("Cell symmetry by international table:  %s" % phono3py._symmetry._international_table)
    print("Supercell symmetry by international table:  %s" % phono3py._phonon_supercell_symmetry._international_table)
...
#    print("phono3py._supercell.get_atomic_numbers():", phono3py._supercell.get_atomic_numbers())
...
#    print(phono3py._displacement_dataset)
    for nn, scell in enumerate(disp_scells):
        if not scell:
            ...
            try:
                with open("phono3py-displacement-%05d/aims.out" % (nn+1), 'r') as f:
                    lines = f.read().splitlines()
                    if "nice day" not in lines[-2]:
                        print("ERROR: phono3py-displacement-%05d/aims.out doesn't have 'nice day'. Leaving now." % (nn+1))
                        sys.exit(-1)
                atoms = read("phono3py-displacement-%05d/aims.out" % (nn+1))
            except:
                print("Cannot read 'phono3py-displacement-%05d/aims.out'\nMake sure that all displacements are calculated." % (nn+1))
                sys.exit(-1)
...
#    print(force_sets)
    ...
    phono3py.run_thermal_conductivity(#grid_points=range(0, mesh[0]*mesh[1]*mesh[2], 10),
                                      # How grid_points are treated: ~/.local/lib/python3.5/site-packages/phono3py/phonon3/conductivity.py
